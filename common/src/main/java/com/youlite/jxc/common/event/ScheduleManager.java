...
public class ScheduleManager {
	...
	private Map<AsyncEvent, EventTimerTask> schedules = Collections
			.synchronizedMap(new HashMap<AsyncEvent, EventTimerTask>());
	...
	class EventTimerTask extends TimerTask {
		...
		EventTimerTask(IAsyncEventListener listener, AsyncEvent event,
				boolean once) {
			super();
	...
	public boolean scheduleTimerEvent(long time, IAsyncEventListener listener,
			AsyncEvent event) {
	...
	public boolean scheduleTimerEvent(Date time, IAsyncEventListener listener,
			AsyncEvent event) {
		boolean result = cancelIfExists(event);
		EventTimerTask task = new EventTimerTask(listener, event, true);
		schedules.put(event, task);
	...
	public boolean scheduleRepeatTimerEvent(long time,
			IAsyncEventListener listener, AsyncEvent event) {
		boolean result = cancelIfExists(event);
